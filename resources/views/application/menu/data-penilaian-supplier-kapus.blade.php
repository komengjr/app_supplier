    ...
    <div class="card-body d-flex justify-content-between">
        <div>
            <select name="agreement" class="form-control choices-single-tahun" id="periode-penilaian" required>
                <option value="">Pilih Penawaran Supplier</option>
                @foreach ($penawaran as $penawarans)
                <option value="{{ $penawarans->data_penawaran_code  }}">{{ $penawarans->data_penawaran_name  }}</option>
                @endforeach
            </select>
...
<script>
    ...
    $(document).on("click", "#button-penilaian-supplier", function(e) {
        e.preventDefault();
        var code = $(this).data("code");
        let data_barang = document.getElementById('data_barang').value;
        let id_penawaran = document.getElementById('periode-penilaian').value;
        if (data_barang == "") {
            Swal.fire({
                icon: "error",
                title: "Oops...",
