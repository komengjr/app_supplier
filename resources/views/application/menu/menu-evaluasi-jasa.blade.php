        ...
        <div class="card bg-100 border">
            <div class="row gx-0 flex-between-center">
                ...
                <div class="col-xl-auto px-3 py-2">
                    ...
                    <h4 class="text-primary fw-bold mb-0">Evaluasi <span class="text-info fw-medium">Jasa</span></h4>
                </div>
            </div>
        </div>
    </div>
    ...
    <div class="card-body d-flex justify-content-between">
        ...
        <div class="d-flex">
            <div class="d-none d-md-block">
                ...
                <a class="btn btn-falcon-warning" href="#!" id="button-cari-jasa"><span class="fas fa-shapes me-1"></span>Pilih Jasa</a>
            </div>
            <div class="dropdown font-sans-serif d-sm-block d-md-none">
                ...
                <div class="dropdown-menu dropdown-menu-end border py-2" aria-labelledby="email-settings">
                    <a class="dropdown-item" href="#!" data-bs-toggle="modal" data-bs-target="#modal-supplier" id="button-tambah-periode-supplier">Tambah Periode</a>
                    <div class="dropdown-divider"></div>
                    <a class="dropdown-item" href="#!" data-bs-toggle="modal" data-bs-target="#modal-supplier" id="button-cari-supplier">Pilih Suplier</a>
...
<script>
    ...
    $(document).on("click", "#button-proses-nilai-supplier", function(e) {
        ...
        $.ajax({
            url: "{{ route('evaluasi_supplier_jasa_pilih_jasa_pilih_supplier_update') }}",
            type: "POST",
            cache: false,
            data: {
                "_token": "{{ csrf_token() }}",
    ...
    $(document).on("click", "#button-tambah-data-supplier", function(e) {
        ...
        if (code == "" || tahun == "") {
            ...
            $.ajax({
                url: "{{ route('evaluasi_supplier_jasa_pilih_jasa_supplier') }}",
                type: "POST",
                cache: false,
                data: {
                    "_token": "{{ csrf_token() }}",
    ...
    $(document).on("click", "#button-simpan-data-supplier", function(e) {
        ...
        var jasa = document.getElementById("code_jasa").value;
        ...
        if (supplier == "") {
            ...
            $.ajax({
                url: "{{ route('evaluasi_supplier_jasa_pilih_jasa_pilih_supplier') }}",
                type: "POST",
                cache: false,
                data: {
                    "_token": "{{ csrf_token() }}",
    ...
    $(document).on("click", "#button-next-periode-penilaian-barang", function(e) {
        ...
        var tahun = document.getElementById("periode-penilaian").value;
        $.ajax({
            url: "{{ route('evaluasi_supplier_jasa_pilih_jasa_pilih_supplier_next') }}",
            type: "POST",
            cache: false,
            data: {
                "_token": "{{ csrf_token() }}",
...
<script>
    $(document).on("click", "#button-simpan-fix-proses-penilaian-jasa", function(e) {
        e.preventDefault();
        ...
        swalWithBootstrapButtons.fire({
            ...
            if (result.isConfirmed) {
                $.ajax({
                    url: "{{ route('evaluasi_supplier_jasa_pilih_jasa_proses_penilaian_supplier_save_fix') }}",
                    type: "POST",
                    cache: false,
                    data: {
                        "_token": "{{ csrf_token() }}",
