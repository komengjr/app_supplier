<div class="modal-body p-0">
    <div class="bg-300 rounded-top-lg py-3 ps-4 pe-6">
        <h4 class="mb-1" id="staticBackdropLabel">Proses Penilaian Supplier <strong class="text-warning">{{ $supplier->m_supplier_name }}</strong> Barang <strong class="text-warning">{{ $jasa->m_jasa_name }}</strong></h4>
        ...
        <div class="accordion" id="accordionExample">
            ...
            <div class="accordion-item">
                ...
                <div class="accordion-collapse collapse show" id="collapse{{$cat->id_t_penilaian_cat }}" aria-labelledby="heading1" data-bs-parent="#accordionExample">
                    <form id="form-jasa{{$cat->t_penilaian_cat_code }}">
                        @csrf
                        <div class="accordion-body">

                            <input type="text" name="supplier" value="{{ $supplier->m_supplier_code }}" id="supplier" hidden>
                            <input type="text" name="jasa" value="{{ $jasa->m_jasa_code }}" id="jasa" hidden>
                                    ...
                                    <tr>
                                        ...
                                        <td>
                                            ...
                                            <select name="data{{ $det->t_penilaian_detail_code  }}" class="form-control" id="">
                                                <option value="{{$log->penilaian_jasa_cab_val}}">{{$log->penilaian_jasa_cab_val}}</option>
                                            ...
                                            @else
                                            @if ($log)
                                            <input type="text" class="form-control" name="data{{ $det->t_penilaian_detail_code  }}" value="{{ $log->penilaian_jasa_cab_val }}">
                                            @else
                            ...
                            <div class="text-end">
                                <button class="btn btn-falcon-success btn-sm" id="button-simpan-prosess-penilaian-jasa{{$cat->t_penilaian_cat_code }}" data-code="{{$cat->t_penilaian_cat_code }}" data-supplier="" data-jasa="{{ $jasa->m_jasa_code }}"><span class="fas fa-save"></span> Save</button>
                            </div>
                        </div>
                    </form>
                </div>
