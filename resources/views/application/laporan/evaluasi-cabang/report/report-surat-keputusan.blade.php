...
@php
$no_cabang = DB::table('master_cabang_no')->where('master_cabang_no_code',$tahun->log_master_cabang)->first();
$no_document = DB::table('log_master')->where('log_master_cabang',$tahun->log_master_cabang)->get();
$nourut = 1;
@endphp
@if ($no_cabang)
@php
...
<body>
    <header class="clearfix" style="padding-bottom: 0px; padding-top: 50px;">
        <div id="company">
            <h5 class="name"><strong>{{$tahun->log_master_no_doc_terpilih}}</strong></h5>
        </div>
    </header>
