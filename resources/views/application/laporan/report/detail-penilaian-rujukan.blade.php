...
<body>
    <header class="clearfix" style="padding-bottom: 0px;">
        <div id="company">
            <h5 class="name"><strong>{{$periode->log_master_no_doc_terpilih}}</strong></h5>
        </div>
    </header>
    <main style="padding-top: 0px;">
        <h3 style="text-align: center; margin: 5px;">LAMPIRAN DETAIL</h3>
        <h3 style="text-align: center; margin: 5px;">DAFTAR RUJUKAN TERPILIH PER JENIS PEMERIKSAAN TAHUN {{$periode->log_master_periode +1}}</h3>
        <table id="example" class="table table-striped border" style="width:100%" border="1">
            ...
            <tbody>
                ...
                <tr>
                    ...
                    <td>
                        {{ $sup->m_rujukan_name }} ( <strong
                            class="text-primary">{{ $sup->data_supp_rujukan_cab_score }}</strong> )
                        <table>
                            <tr>
                                @foreach ($cat as $cats)
                            ...
                            <tr>
                                @foreach ($cat as $cats)
                                @php
                                $point = DB::table('log_penilaian_rujukan_cab')
                                ...
                                ->where('log_penilaian_rujukan_cab.log_master_code',$periode->log_master_code)
                                ->where('log_penilaian_rujukan_cab.m_rujukan_code',$sup->m_rujukan_code)
